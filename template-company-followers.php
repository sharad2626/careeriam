<?php
/**
 * Template name: Company Followers
 */
...
<?php get_template_part('header-myaccount');
			...
			<!-- filters start  -->
				...
				<div class="filters" >
						...
						<div class="full sidebar-widget-bottom-line" style="display:none">
							...
							//print_r($wpjobus_companies_est_year);
		  ...
		  <div class="listing-results" id="companies-block-list-ul">
			...
			<span class="text">
			  ...
			  global $companies_per_page, $total_companies, $total_pages, $current_page;
								...
								$myquery = " SELECT T1.* FROM wp_posts T1 INNER JOIN wp_postmeta T2 ON T1.post_author = T2.meta_value WHERE T2.meta_key='company-followers' AND T2.post_id = $c_id AND T1.post_type='resume'";
			...
			<?php foreach ($wpjobus_companies as $key ) {
					$current_pos++;
				...
				if($current_page == 1) {
									...
									$current_element_id++;
										...
										$wpjobus_resume_fullname = esc_attr(get_post_meta($post_id, 'wpjobus_resume_fullname',true));
			...
			<div class="result-list" id="ok">
			       ...
			       <a class="resumelists" href="<?php $companylink = home_url()."/resume/".$post_id; $register_link = home_url()."/register/";  if(is_user_logged_in()){ echo $companylink; }else{ echo $register_link;  } ?>">
					...
					<div class="list-icon">
					  <!-- <img src="<?php echo get_template_directory_uri(); ?>/images/member-icon.png" alt="Member Icon" /> -->
						...
						$params = array( 'width' => 85, 'height' => 85, 'crop' => true );
							$wpjobus_resume_profile_picture = esc_url(get_post_meta($key->ID, 'wpjobus_resume_profile_picture',true));
					...
					<div class="list-details">
					  <div class="title">
						<h3>
							<?php echo $wpjobus_resume_fullname;  		
			             ?>
						</h3>
						<span class="designation"><?php echo $resume_industry = esc_attr(get_post_meta($key->ID, 'resume_industry',true));?></span>
					  </div>
					  <div class="loc">
						<span><?php  echo $resume_location = get_post_meta($key->ID, 'resume_location',true); ?></span>
					  </div>
					</div>
					</a>
					<div class="list-action" >
					  ...
					  <?php if(is_page('company-followers')){
					  		echo $company_user_id;
...
<!-- end changes by prajna -->
		...
		<input type="hidden" id="followers_id" name="followers_id" value="<?php echo $fid;?>" />
		<input type="hidden" id="current_page" name="current_page" value="<?php echo $current_page;?>" />
		<input type="hidden" name="action" value="wpjobusSubmitResumesFilterNew" />
		<?php //wp_nonce_field( 'wpjobusSubmitResumesFilter_html', 'wpjobusSubmitResumesFilter_nonce' ); ?>
		<?php wp_nonce_field( 'wpjobusSubmitResumesFilterNew_html', 'wpjobusSubmitResumesFilterNew_nonce' ); ?>
	    ...
	    <script type="text/javascript">
					...
					jQuery(function($) {
						...
						jQuery(".pagination a.page-numbers").click(function(e){
					     		...
					     		var hrefprim = jQuery(this).attr('href');
					     	var href = hrefprim.split('#');
					     	//alert(har[1]);

					     	//var href = hrefprim.replace("#", "");
					     	
	                		jQuery('#companies_current_page').val(href[1]);
